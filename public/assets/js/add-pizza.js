...
const handlePizzaSubmit = event => {
  ...
  fetch('/api/pizzas', {
    ...
    .catch(err => {
      console.log(err);
      saveRecord(formData);
      //when a user tries to submit a new pizza without an internet connection, instead of the app failing
      //data will be stored in the new_pizza object store in the browser's IndexedDB database called pizza_hunt
    });
    //What we've done here is POST the formData object to the API
};
